...
| `PlatformDescriptor` | What a config module declares in its `META-INF/bdi-contract-platform.json`: the Quarkus keys of the extensions it configures, and key templates for the channels (`{channel}`), REST clients and gRPC clients (`{client}`) an application declares by injection (`{application}` substituted too). Written once per module by the platform team, never per application. |

...
| `bdi-quarkus`: BOM, application parent (`bdi-quarkus-parent`, tested with applications outside the reactor), 22 starters, 17 config modules | done; IBM MQ, security, secrets, SOAP and Spring variations planned with their POCs |
...
| POC remote EJB | done: provider (JAX-RS façade + gRPC over the ex-remote bean) and caller (REST client with token propagation, service identity, gRPC stub, idempotent remote step); the `.proto` and client interfaces as the interface jar |
